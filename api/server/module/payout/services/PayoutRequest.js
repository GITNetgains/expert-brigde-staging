...
exports.sendRequest = async (tutorId, payoutAccount) => {
  try {
    ...
    if (payoutRequest) {
      if (moment(payoutRequest.updatedAt).isSame(moment(), 'day')) {
        if (payoutRequest.requestAttempts >= process.env.MAX_PAYOUT_REQUEST_PER_DAY) {
          throw new Error('Your payout request has been sent aldready');
        } else {
    ...
    // send email to admin
    await Service.Mailer.send('payout-request-to-admin', process.env.EMAIL_NOTIFICATION_PAYOUT_REQUEST, {
...
exports.approveRequest = async (requestId, options) => {
  try {
    ...
    await Service.Mailer.send('payout-approve-notify-to-tutor', tutor.email, {
      subject: `Payment request #${payoutRequest.code} has been approved`,
      tutor: tutor.toObject(),
      payoutRequest,
      subject_replace_fields: {
        payoutRequestCode: payoutRequest.code
      }
    });

    const notification = {
      title: 'Payout request - Approve request',
      description: `Payout request #${payoutRequest.code} has been approved`,
      itemId: payoutRequest._id,
      notifyTo: tutor._id,
      type: 'payout'
    };
...
exports.rejectRequest = async (requestId, options) => {
  try {
    ...
    const notification = {
      title: 'Payout request - Reject request',
      description: `Payout request #${payoutRequest.code} was rejected`,
      itemId: payoutRequest._id,
      notifyTo: tutor._id,
      type: 'payout'
    };
