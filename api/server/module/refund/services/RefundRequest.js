...
exports.sendRequest = async (userId, options) => {
  try {
    ...
    await Service.Mailer.send('refund-request-to-admin', process.env.EMAIL_NOTIFICATION_PAYOUT_REQUEST, {
    ...
    await Service.Mailer.send('refund-request-to-admin', tutor.email, {
      subject: `Refund request from ${user.name}`,
...
exports.approveRequest = async (refundRequest, options) => {
  try {
    ...
    await Service.Mailer.send('refund-approve-notify-to-user', user.email, {
      subject: `Refund request #${refundRequest.code} has been approved`,
      user: user.toObject(),
      refundRequest,
      subject_replace_fields: {
        refundRequestCode: refundRequest.code
      }
    });
    ...
    const notification = {
      ...
      itemId: refundRequest._id,
      notifyTo: user._id,
      type: 'refund'
    };
    await Service.Notification.create(notification);

    await Service.Mailer.send('refund-approve-notify-to-tutor', tutor.email, {
...
exports.rejectRequest = async (refundRequest, options) => {
  try {
    ...
    await Service.Mailer.send('refund-reject-notify-to-user', user.email, {
...
exports.confirmRefunded = async (refundRequest, options) => {
  try {
    ...
    // );
    const user = await DB.User.findOne({ _id: refundRequest.userId });
    await Service.Mailer.send('refund-refund-notify-to-user', user.email, {
    ...
    const notification = {
      title: `Refund request`,
      description: `Your refund request has been completed`,
      itemId: refundRequest._id,
      notifyTo: user._id,
      type: 'refund'
    };
